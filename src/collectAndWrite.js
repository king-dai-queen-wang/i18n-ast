  ...
  // 通过output的路径 得到翻译过的文字
    ...
    async autoTranslate(additionalTranslateWords, option, localName, callback) {
      ...
      if(translateConfig?.name === 'BaiduTranslate') {
        ...
        await bdt.translate(JSON.stringify(additionalTranslateWords), targetLang, 'zh').then(async function(res) {
          console.log(res)
    ...
    execOtherLocales: async function(additionalTranslateWords, option,localName, callback) {
      console.log(additionalTranslateWords, option);
      
      if (this.option.autoTranslate?.enable && Object.keys(additionalTranslateWords).length) {
        await this.autoTranslate(additionalTranslateWords, option, localName, callback)
      } else {
        callback(null)
      }
    ...
    start: async function() {
      ...
      if(this._existsSync(outputMainLocalPath(this.option.mainLocal))) {
        Object.assign(allTranslateWords, this.getExistWords(outputMainLocalPath(this.option.mainLocal)).keyValue);
      }
      // 得到需要翻译文件 的 路径
      ...
      if(this.option.otherLocales) {
        // 生成其他语言文件
      const buffer = this.option.otherLocales.map((localName) => new Promise(async (resolve, reject) => {
          // 得到要输出文件的 路径
          const path = outputMainLocalPath(localName);
          
          // 处理其他的翻译文件，增量判断 要翻译的，回调函数把最后的数据 写文件
          await this.execOtherLocales(additionalTranslateWords, option,localName, async (additionalWords) => {
            if(additionalWords === null && Object.keys(additionalTranslateWords).length) {
              const additionalEmptyVal =  new Proxy(additionalTranslateWords, {get: () => ""})
              additionalWords = JSON.parse(JSON.stringify(additionalEmptyVal))
            }
          ...
          // 将其他语言文件 已翻译过的 读取出来 key-val
            ...
            // 合并 所有 翻译的单词 key -> val
            const allWords = Object.assign({}, existWords, additionalWords);
      ...
      // 写入主语言 文件
      await this.write(outputMainLocalPath(this.option.mainLocal), reorganizeContent, { encoding: "utf-8" });
    }
  }
